...
void aux_percorre_intersec(NO_ARV* no_atual, conjun* C2, conjun* novo_conjun){
    if(!no_atual)
      return;
    ITEM* item_atual = retorna_item_no_ARV(no_atual);
    ITEM* novo_item;
    if(conjun_pertence(C2,item_atual)){
       novo_item = item_copiar(item_atual);
       conjun_add_item(novo_conjun, novo_item);
    }

    aux_percorre_intersec( no_avl_pega_dir(no_atual) ,C2, novo_conjun);
    aux_percorre_intersec( no_avl_pega_esq(no_atual) ,C2, novo_conjun);
}
...
void conjunto_intersec(conjun* C1, conjun* C2, conjun* novo_conjun){
    ...
    aux_percorre_intersec( avl_pega_raiz(C1->arvo_elem) , C2, novo_conjun );
}

void aux_percorre_uniao_segundo_conjun(NO_ARV* no_atual, conjun* novo_conjun){
    if(!no_atual)
      return;
    ITEM* item_atual = retorna_item_no_ARV(no_atual);
    ITEM* novo_item;
    ...
    aux_percorre_uniao_segundo_conjun( no_avl_pega_dir(no_atual) , novo_conjun);
    aux_percorre_uniao_segundo_conjun( no_avl_pega_esq(no_atual) , novo_conjun);
}
...
void conjunto_uniao(conjun* C1, conjun* C2, conjun* novo_conjun){
    ...
    aux_percorre_uniao_primei_conjun( avl_pega_raiz(C1->arvo_elem), novo_conjun );
    aux_percorre_uniao_segundo_conjun( avl_pega_raiz(C2->arvo_elem), novo_conjun );        
}
